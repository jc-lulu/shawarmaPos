...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkQuery = "SELECT quantity, productType FROM inventory WHERE productId = ? AND type = 0";
    ...
    if ($result->num_rows > 0) {
        ...
        try {
            ...
            $insertQuery = "INSERT INTO inventory (productName, quantity, productType, type, requestedBy, transactionStatus, dateOfOut) VALUES (?, ?, ?, 1, 0, 1, ?)";
            $insertStmt = $connection->prepare($insertQuery);
            $insertStmt->bind_param("siss", $productName, $quantity, $productType, $dateOfOut);
            $insertStmt->execute();
